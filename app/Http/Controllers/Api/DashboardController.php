...
class DashboardController extends Controller
{
    ...
    public function getInvoiceStats(Request $request){
        ...
        $client_id = auth()->user()->id;
        $total_invoices = Invoice::where('user_id',$client_id)->whereNotIn('status', ["draft", "discard"])->whereBetween(DB::raw('date(created_at)'), [$request->start_date, $request->end_date])->count();
        
        $total_paid_invoices = Invoice::where('user_id',$client_id)->whereNotIn('status', ["draft", "discard"])->whereBetween(DB::raw('date(created_at)'), [$request->start_date, $request->end_date])->where('status','paid')->count();
        $total_unpaid_invoices = Invoice::where('user_id',$client_id)->whereNotIn('status', ["draft", "discard"])->whereBetween(DB::raw('date(created_at)'), [$request->start_date, $request->end_date])->where('status','unpaid')->count();
        
        $capture_rate = ($total_invoices>0)? round(($total_paid_invoices/$total_invoices)*100):0;
        
       

        $total_office_paid_invoices = Invoice::where('user_id',$client_id)->whereNotIn('status', ["draft", "discard"])->whereBetween(DB::raw('date(created_at)'), [$request->start_date, $request->end_date])->where('status','paid')->where('payment_mode','office')->count();
        $total_online_paid_invoices = Invoice::where('user_id',$client_id)->whereNotIn('status', ["draft", "discard"])->whereBetween(DB::raw('date(created_at)'), [$request->start_date, $request->end_date])->where('status','paid')->where('payment_mode','online')->count();
